...
var page_config = {
    ...
    NEXT_AND_PREV_LINK: require("../../testing_url").NEXT_AND_PREV_LINK
}
var file_name;
...
Object.keys(page_config.NEXT_AND_PREV_LINK).map((value, index) => {
    let test_case_details = page_config.NEXT_AND_PREV_LINK[value]
    let copy_test = Object.assign({
        [`${index} ${test_case_details.tag}`]: function (client) {
            ...
            client.source(function (res) {
                ...
                client.source(function (res) {
                    test_case_details["testCase"].map((v, i) => {
                        client.assert.NextAndPrev(v)
                    })
                })
            })
            client.end();
